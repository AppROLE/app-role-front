...
import { RefreshTokenResponse } from './types/user_dto';
  ...
  async (error) => {
    ...
    if (error.response && error.response.status === 401 && error.response.data.message === "The incoming token has expired") {
      ...
      try {
        ...
        const response = await http.post<RefreshTokenResponse>(`${process.env.EXPO_PUBLIC_API_URL}/refresh-token`, undefined, {
