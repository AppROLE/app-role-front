    ...
    <Background>
      <View className="relative w-full flex-1">
        <View className="absolute top-0 flex h-12 w-full flex-row items-center gap-3 border-b-2 border-b-[#2C2B2B] pb-8">
          ...
          >
            <SvgUri source={{uri: process.env.EXPO_PUBLIC_URL_S3 + "/calendar_gradient.svg"}} width={32} height={32} />
          </TouchableOpacity>
        ...
        <View className="flex-1">
          <ScrollView className="mt-12 flex-1">
            ...
            {filters.map((filter, index) => (
              <View
                key={index}
                className={`'border-t-2 mb-8 flex flex-col gap-2 border-t-[#2C2B2B] pb-2 pt-5 border-t-2`}
              >
                <View className="flex flex-row">
                  <View className="ml-4 mt-4">
