...
export default function Home() {
    ...
    return (
        <Background text={phrase} scrollable lockScroll={scrollDisabled} function1={loadMoreRoles}>
            <Text className="text-white text-3xl font-nunitoBold text-center mb-4">Role Bombando</Text>
            <View className="px-12 mb-10">
                ...
                <View className="flex flex-row justify-center gap-2 mt-4">
                  {carrosselData.map((_, index) => (
                    <View
                      key={index}
                      className={`h-[6px] rounded-full ${activeSlide === index ? 'bg-white w-[18px]' : 'bg-[#BDBDBD] w-[6px]'}`}
                    />
                  ))}
                </View>
            </View>
            ...
            <View className="px-12 flex flex-col gap-4">
                <Text className="text-white text-3xl font-nunitoBold">Explore</Text>
                {roles.slice(0, 5).map((role, index) => (
                    <RoleCard key={`id${role.idRole}ind${index}`} {...role} />
                ))}
                <View>
                    <Text className="text-white text-2xl font-nunitoBold mt-3 mb-2">Categorias</Text>
                ...
                <View>
                    <Text className="text-white text-2xl font-nunitoBold mt-3 mb-2">Gênero Musical</Text>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{marginBottom: 16}}>
                ...
                <View className="mt-8 pb-8">
                    <Text className="text-lg text-[#BDBDBD] text-center font-nunito">Não encontrou o que procurava?</Text>
                    <Text className="text-lg text-[#BDBDBD] text-center font-nunito">Utilieze os nossos <Text className="font-nunitoBold text-white">Filtros!</Text></Text>
